...
function SidebarIcon({ name }: { name: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    emaillogs: <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" /></svg>,
  }
...
const navItems = [
  ...
  { name: 'Email Logs', icon: 'emaillogs', href: '/dashboard/email-logs' },
    ...
    <aside className="sidebar">
        ...
        {navItems.map((item) => {
          ...
          if (currentProject) {
              if (resolvedHref.includes('?')) {
                  resolvedHref = `${resolvedHref}&project=${currentProject}`
              } else {
                  resolvedHref = `${resolvedHref}?project=${currentProject}`
              }
          }

          // Very simple heuristic to highlight the active menu item
          ...
          return (
            <Link
              key={item.name}
              href={resolvedHref}
              className={`sidebar-link ${isActive ? 'bg-slate-800 text-white' : ''}`}
            >
      ...
      <div className="sidebar-footer">
        ...
        <form action="/auth/signout" method="post">
          <button className="text-xs text-slate-500 hover:text-red-400 mt-2 transition-colors w-full text-left">
            Sign Out
          </button>
        </form>
      </div>
